...
public final class PortalRegistry {
     ...
     *
     * @param world The {@link Level} the portal is located in.
     * @param pos   The {@link BlockPos} of the a portal frame.
     * @param side  The {@link Direction} representing the side of the portal frame that was hit
     *              by the portal activator.
     * @return <code>true</code> if a portal could be activated, otherwise <code>false</code>.
     */
    public static boolean activatePortal(Level world, BlockPos pos, Direction side) {
        ImmutableMap<Direction, Direction[]> cornerSearchDirs = PortalRegistry.cornerSearchDirs;
        if (world == null || pos == null || side == null || cornerSearchDirs == null) return false;
        ...
        corner4 = findCorner(world, corner3.getPos().relative(firstSearchDir.getOpposite()), firstSearchDir.getOpposite(), side.getOpposite());
        ...
        for (BlockPos portalPos : portalPositions) {
            world.setBlockAndUpdate(portalPos, SimplePortals.blockPortal.defaultBlockState().setValue(BlockPortal.AXIS, portalAxis));
        }
     ...
     *
     * @param world The {@link Level} the portal is located in.
     * @param pos   The {@link BlockPos} of one of the portals blocks (portal or frame).
     */
    public static void deactivatePortal(Level world, BlockPos pos) {
     ...
     * @return <code>true</code> if the block at the specified position is part of
     ...
     *
     * @return An immutable map containing all portals and their addresses.
     */
     ...
     * @return A read-only list of found portals (may be empty) or <code>null</code> if
     * <code>pos</code> was <code>null</code>.
     */
    public static List<Portal> getPortalsAt(BlockPos pos, ResourceKey<Level> dimension) {
        if (pos == null) return null;

        return portals.get(pos).stream().filter(portal -> portal.getDimension().equals(dimension)).toList();
    }
     ...
     *
     * @param address An portal {@link Address}.
     * @return A read-only list of found portals (may be empty) or <code>null</code> if
     ...
     *
     * @param portal The {@link Portal} to get the power gauges for.
     * @return A read-only list of {@link BlockPos} (may be empty) or <code>null</code> if
     ...
     *
     * @param portal The {@link Portal} to which the power should be added.
     * @param amount The amount of power to add.
     * @return The surplus power that could not be added to the portal.
     */
     ...
     *
     * @param portal The {@link Portal} from which the power should be removed.
     * @param amount The amount of power to remove.
     * @return <code>true</code> if the amount could be removed, otherwise <code>false</code>.
     */
     ...
     *
     * @param portal The {@link Portal} to get the power for.
     * @return The portals power or <code>0</code>.
     */
     ...
     */
    public static void updatePowerGauges(Level world, Portal portal) {
        if (world == null | portal == null) return;

        List<BlockPos> gaugePositions = getPowerGauges(portal);

        for (BlockPos pos : gaugePositions) {
            world.blockUpdated(pos, SimplePortals.blockPowerGauge);
        }
    }
     ...
     *
     * @param block The {@link Block} to generate the address id for.
     * @return A string of the format "registryName#meta" or <code>null</code>
     ...
     * @param portal The {@link Portal}.
     */
    private static void destroyPortalBlocks(Level world, Portal portal) {
     ...
     * @return The distance or <code>-1</code> if either position was <code>null</code>.
     */
     ...
     * @return A {@link Corner} or <code>null</code> if one of the parameters was <code>null</code> or
     * no corner could be found.
     */
    private static Corner findCorner(Level world, BlockPos startPos, Direction searchDir, Direction cornerFacing) {
     ...
     */
    private static void register(Level world, Portal portal, List<BlockPos> powerGauges) {
        if (world == null || portal == null) return;
     ...
     */
    private static void unregister(Level world, Portal portal) {
        if (world == null || portal == null) return;
     ...
     * @return <code>true</code> if the block is a portal frame (power gauges also count as part of the frame), otherwise <code>false</code>.
     */
    private static boolean isPortalFrame(Level world, BlockPos pos) {
     ...
     * @return <code>true</code> if the block is valid, otherwise <code>false</code>.
     */
     ...
     *
     * @param nbt The {@link CompoundTag} to save the registry data in.
     */
     ...
     *
     * @param nbt The {@link CompoundTag} to read the registry data from.
     */

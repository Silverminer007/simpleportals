...
public final class PortalRegistry
{
	...
	public static boolean activatePortal(World world, BlockPos pos, Direction side)
	{
		...
		if (corner4 == null || !corner4.equals(findCorner(world, corner2.getPos().relative(side), side, firstSearchDir))) return false;
	...
	public static List<Portal> getPortalsAt(BlockPos pos, RegistryKey<World>  dimension)
	{
		...
		List<Portal> foundPortals = portals.get(pos).stream().filter(portal -> portal.getDimension().equals(dimension)).collect(Collectors.toList());

package net.zarathul.simpleportals_reloaded.configuration.gui;
...
public class EnumOptionButton<E extends Enum<E>> extends ExtendedButton
{
	...
	{
		super(x, y, width, height, new StringTextComponent(I18n.get(I18N_ENUM_PREFIX + clazz.getSimpleName().toLowerCase() + "." + value.toLowerCase())), (button) -> {;});
		...
		{
			names[i] = e.name();
			i18nNames[i] = I18n.get(I18N_ENUM_PREFIX + clazz.getSimpleName().toLowerCase() + "." + e.name().toLowerCase());

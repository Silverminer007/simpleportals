...
import net.zarathul.simpleportals_reloaded.commands.arguments.BlockArgument;
import net.zarathul.simpleportals_reloaded.configuration.Config;
import net.zarathul.simpleportals_reloaded.registration.Address;
...
public class CommandPortals {
	...
	private static int cooldown(CommandSource source, ServerPlayerEntity target) {
		...
		SendTranslatedMessage(source, "commands.sportals.cooldown.success", target.getName(),
				target.getDimensionChangingDelay(), target.getDimensionChangingDelay() / 20f); // This assumes normal
																								// tickrate of 20 (TPS).

...
public class Portal implements INBTSerializable<CompoundNBT> {
	 ...
	 * 
	 * @return The portals {@link Address} or <code>null</code>.
	 */
	...
	public BlockPos getPortDestination(World world, int entityHeight) {
		...
		Direction searchDirPositive = Direction.get(AxisDirection.POSITIVE, cornerAxis);
		Direction searchDirNegative = Direction.get(AxisDirection.NEGATIVE, cornerAxis);
	...
	public boolean isDamaged(World world) {
		...
		for (BlockPos pos : getPortalPositions()) {
			if (!(world.getBlockState(pos).getBlock() instanceof BlockPortal))
				return true;
		}
